...
"""
from ctypes import *
from decisionTree import Classify as decisiontree
import math
import random
import os
...
def cls(label, width, height):
    if (label == "Screw"):
        return decisiontree(1, width, height, width*width + height*height)
    else:
        return decisiontree(2, width, height, width*width + height*height)
...
def draw_boxes(detections, image, colors):
    ...
    for label, confidence, bbox in detections:
        ...
        size = cls(label, bbox[2], bbox[3])
        size_copy.append(size)
        cv2.putText(image, "{} [{:.2f}]{}".format(label, float(confidence), size),
                    (left, top - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    colors[label], 2)
    return image,size_copy
...
lib = CDLL("/home/aaronace/Downloads/darknet_L4/libdarknet.so", RTLD_GLOBAL)
